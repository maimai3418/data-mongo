...
彙整多個 IQ xlsx 資料檔，確認「應該是同一筆紀錄」的資料不會被重複匯入資料庫：
  1. 以 famid + age 作為重複判定的 key。
     欄位名稱一律先轉小寫再比對（famid / famID 視為相同）；
     age 欄位另可接受別名 age_IQ / age_iq。
  2. key 相同時，比對欄位除了 sex, birth_date, record_date（與 key 本身
     famid、age）以外的所有欄位：
       - 全部相同 → 視為同一筆，只保留一筆（合併記錄所有資料來源）。
       - 有任何差異 → 視為不同筆，全部保留（各自標記資料來源與差異欄位）。
  3. 輸出 iq_precheck.xlsx：
...
與 CPT/CANTAB precheck 的差異：IQ 目前沒有 field.json（iq/fields/ 不存在，
無 codebook 可依循）。若之後補上 iq/fields/*iq*fields.json（格式同 CPT/CANTAB），
本腳本會自動改用該檔案定義的欄位順序與比對範圍；找不到時則退回「資料驅動」模式：
比對欄位＝本次讀到的所有欄位，扣掉 famid/age/sex/birth_date/record_date
（CLAUDE.md §7 原則5：資料驅動而非 protocol 驅動，不可靠的文件不應阻擋處理）。

用法（從專案根目錄執行）：
  python iq/iq_precheck.py <檔案或資料夾>
  python iq/iq_precheck.py data_dir/
  python iq/iq_precheck.py a.xlsx b.xlsx
  python iq/iq_precheck.py data_dir/ -o report.xlsx
...
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "iq_precheck.xlsx")

# 重複判定 key（欄位名稱一律先轉小寫再比對，famid / famID 視為相同）
KEY_FIELDS = ["famid", "age"]
# age 欄位在資料檔中可能的別名（小寫），讀檔時統一改名為 age
AGE_ALIASES = ("age_iq", "age")
# 不參與比對的欄位（famid/age 為 key 本身，另外排除下列 metadata 欄位）
EXCLUDE_COMPARE = {"famid", "sex", "birth_date", "record_date"}
# ────────────────────────────────────────────────────────────────────────

# ---- 報表樣式（沿用 cpt_precheck.py / cantab_precheck.py 的風格）----
FONT_NAME = "Arial"
...
# ── field.json（選用；找不到時退回資料驅動模式）──────────────────────────
def discover_field_json(fields_dir=FIELDS_DIR):
    """找出 IQ 的 field.json：檔名（小寫）含 'iq' 且以 'fields.json' 結尾；
    多個取檔名最大者（日期前綴最新）。找不到資料夾或檔案回傳 None。"""
    if not os.path.isdir(fields_dir):
        return None
    matches = sorted(
        f for f in os.listdir(fields_dir)
        if f.lower().endswith("fields.json") and "iq" in f.lower()
    )
    return os.path.join(fields_dir, matches[-1]) if matches else None


def load_compare_fields():
    """回傳 (json 路徑或 None, 全欄位清單或 None, 比對欄位清單或 None)。

    找不到 field.json 時全部回傳 None，由呼叫端改用資料驅動模式
    （比對欄位 = 本次實際讀到的欄位，扣掉 EXCLUDE_COMPARE 與 key 欄位）。
    """
...
def read_records(files):
    ...
    for filepath in files:
        ...
        for sheet_name, df in all_sheets.items():
            ...
            df = df.copy()
            bad_cols = [c for c in df.columns if not isinstance(c, str)]
            if bad_cols:
...
def dedupe(records, compare_fields):
    ...
    回傳：
      ...
      conflicts   差異保留群組 [(key, [(signature, recs)], diff_fields)]
      missing     缺 key 的紀錄清單
    """
    groups = OrderedDict()
    missing = []
...
# ── 輸出 ────────────────────────────────────────────────────────────────
def build_report(output_path, files, field_source_label, all_cols_for_display,
                 compare_fields, records, all_cols, read_errors, kept,
                 dup_groups, conflicts, missing):
    ...
    ws["A2"] = (f"重複判定 key：{' + '.join(KEY_FIELDS)}｜"
                f"比對欄位：{len(compare_fields)} 個（{field_source_label}，"
                f"排除 {', '.join(sorted(EXCLUDE_COMPARE))}）")
    ...
    data_cols = [c for c in all_cols_for_display if c in present and c != "famid"]
    data_cols += [c for c in all_cols
                  if c not in all_cols_for_display and c not in ("famid", "age")]
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="IQ 匯入前彙整檢查：依 famid+age 找出重複資料並彙整")
...
def main(argv=None):
    ...
    json_path, json_fields, json_compare = load_compare_fields()

    records, all_cols, read_errors = read_records(files)
    for filename, err in read_errors:
        print(f"⚠️  {filename}: {err}")
    print(f"📄 檔案數：{len(files)}｜共讀取 {len(records)} 列")

    if json_path:
        compare_fields = json_compare
        all_cols_for_display = json_fields
        field_source_label = os.path.basename(json_path)
        print(f"📐 比對欄位來源：{json_path}")
    else:
        # 資料驅動模式：field.json 不存在，比對欄位 = 本次讀到的欄位扣掉 key/metadata
        compare_fields = [c for c in all_cols if c not in EXCLUDE_COMPARE and c != "age"]
        all_cols_for_display = all_cols
        field_source_label = "資料驅動：無 iq/fields/*.json，比對欄位取自本次讀到的欄位"
        print(f"📐 找不到 IQ field.json（{FIELDS_DIR}），改用資料驅動模式")
    ...
    summary_rows = build_report(
        args.output, files, field_source_label, all_cols_for_display,
        compare_fields, records, all_cols, read_errors, kept, dup_groups,
        conflicts, missing)
