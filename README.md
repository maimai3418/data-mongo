...
| `python tools/tree.py <dir>`        | 匯出資料夾結構                             | `directory_tree.xlsx`            |
...
### CPT 原始檔解析（cpt/cpt_convert.py）

把 CPT 匯出的原始 txt 解析成 xlsx。每筆紀錄固定為 8 行的區塊，逐行拆出受測者資訊（自動偵測有無 `name` 欄）、用藥資訊與各項分數欄位，輸出單一 sheet（`CPT`）的 xlsx。

```bash
python cpt/cpt_convert.py                 # 預設讀 cpt_raw.txt，輸出 cpt_parsed.xlsx
python cpt/cpt_convert.py cpt_raw.txt     # 指定要解析的 txt
python cpt/cpt_convert.py cpt_raw.txt -o out.xlsx   # 指定輸出檔名
```

- `famid` / `famid_dup` 字串化並去除尾端 `.0`；數值欄自動轉型，全空的 `empty_` 欄自動丟棄
- 欄位順序整理為受測者資訊在前、分數欄位在後

### 資料夾結構匯出（tools/tree.py）

將資料夾結構（含層級、類型、副檔名、大小、相對路徑）匯出為 Excel。

```bash
python tools/tree.py <dir>
python tools/tree.py <dir> -o directory_tree.xlsx --ignore .git node_modules
```
