...
# -*- coding: utf-8 -*-
"""
CANTAB 匯入：讀取 CANTAB xlsx，依 famid + record_date 做三層去重複後匯入 CANTAB collection。

三層去重複（CLAUDE.md §5，同 CPT）：
...
沒有 record_date 的列，分兩種情況（CLAUDE.md §3、§9）：
  - 有年齡欄位（age_cantab / age，大小寫不拘，field.json 未定義此欄位）：
    record_date=null，標記 age_suspect=true，年齡存入 raw_cantab_age。
  - 完全沒有年齡也沒有日期：record_date=null，標記 flag_no_date=true（同 CPT 慣例）。
  兩者皆不強制配對日期、不套用三層 key 比對邏輯，只做「famid + 量表欄位值
  （age_suspect 情況下含年齡）簽章是否與既有同類型 null-date 紀錄完全相同」的
  hard dedup；不同 → 一律新增（保留為獨立紀錄）。所有處理過的列皆記錄到
  no_date_records.xlsx，供之後 famid_timepoint_map.py（日期回填管線，尚未實作）
  人工/自動回填參考。

CANTAB 沒有 999 null sentinel 慣例，不做全域 999→null 轉換（見 cantab_config.py）。
famid 在 CANTAB 語境直接等同個人 ID（CLAUDE.md §2）。

用法（從專案根目錄執行）：
  python cantab/cantab_import.py <檔案或資料夾>
  python cantab/cantab_import.py data_dir/
  python cantab/cantab_import.py a.xlsx b.xlsx
  python cantab/cantab_import.py data_dir/ --dry-run
  python cantab/cantab_import.py a.xlsx --project-code NHRI113
  python cantab/cantab_import.py a.xlsx -e errors.xlsx --conflict-output conflicts.xlsx
...
DEFAULT_ERROR_OUTPUT = f"{TODAY}_CANTAB_import_error.xlsx"
DEFAULT_CONFLICT_OUTPUT = f"{TODAY}_CANTAB_conflict_report.xlsx"
...
def convert_item(col, val, rule):
    ...
    無 rule 原樣上傳（能轉數字就存數字）。不做 999→null 轉換（CANTAB 無此慣例）。"""
    if is_blank(val):
        return None, None
    s = str(val).strip()
    if s == "":
...
def norm_value(v):
    ...
    讓 "5"、"5.0"、5、5.0 視為相同值（同 cpt_precheck.py / cantab_precheck.py）。"""
    if v is None:
        return None
...
def find_age_column(columns):
    """大小寫不拘找出年齡欄位（age_cantab / age），field.json 未定義此欄位。"""
...
def process_sheet(filename, sheet_name, df, errors, field_rules):
    ...
    回傳 (dated_records, no_date_records, n_empty)：
      ...
      no_date_records 中的 doc 可能含 raw_cantab_age（原始年齡值，尚未分類 flag）。
    """
    df = df.copy()
    ...
    header_set = set(CANTAB_HEADER_FIELDS)
    age_col = find_age_column(df.columns)
    item_cols = [c for c in df.columns if c not in header_set and c != age_col]
    ...
    for _, row in df.iterrows():
        ...
        if doc.get("record_date"):
            dated_records.append({"source": source, "doc": doc})
        else:
            if age_col is not None:
                raw_age, err = None, None
                age_val = rowd.get(age_col)
                if not is_blank(age_val):
                    try:
                        raw_age = float(str(age_val).strip())
                    except (ValueError, TypeError):
                        err = f"{age_col} 年齡格式錯誤"
                if err:
                    errors.append({
                        "file": filename, "sheet": sheet_name,
                        "famid": doc.get("famid"), "record_date": None,
                        "error": err,
                    })
                    continue
                if raw_age is not None:
                    doc[RAW_AGE_FIELD] = raw_age
            no_date_records.append({"source": source, "doc": doc})
...
def reconcile(dated_records, no_date_records, compare_fields, db):
    """回傳 dict：
      to_insert          [doc, ...]                 待新增（含 no-date / age_suspect）
    ...
    for rec in dated_records:
        key = tuple(rec["doc"][f] for f in CANTAB_SHARED_FIELDS)
    ...
    existing_nodate_sigs = {}    # famid -> set(signature)  （flag_no_date 紀錄）
    existing_agesuspect_sigs = {}  # famid -> set(signature)（age_suspect 紀錄，簽章含 raw_cantab_age）
    if all_famids:
        for existing in db[CANTAB_COLLECTION].find({"famid": {"$in": all_famids}}):
            fid = existing.get("famid")
            rdate = existing.get("record_date")
            if rdate:
                existing_dated.setdefault((fid, rdate), existing)
            elif existing.get("age_suspect"):
                sig = build_signature(existing, compare_fields) + (existing.get(RAW_AGE_FIELD),)
                existing_agesuspect_sigs.setdefault(fid, set()).add(sig)
            else:
    ...
    # ── no-date：famid + 簽章 hard dedup（不比對日期），依是否有年齡分兩種旗標 ──
    no_date_report = []
    seen_nodate_this_run = {}      # famid -> set(signature)
    seen_agesuspect_this_run = {}  # famid -> set(signature+age)
    for rec in no_date_records:
        doc = rec["doc"]
        fid = doc["famid"]
        has_age = doc.get(RAW_AGE_FIELD) is not None

        if has_age:
            sig = build_signature(doc, compare_fields) + (doc.get(RAW_AGE_FIELD),)
            known = existing_agesuspect_sigs.get(fid, set()) | seen_agesuspect_this_run.get(fid, set())
            if sig in known:
                no_date_report.append({
                    "collection": CANTAB_COLLECTION,
                    "reason": "無 record_date，僅有年齡，且與既有 age_suspect 紀錄完全相同，略過",
                    **doc,
                })
                continue
            doc["age_suspect"] = True
            to_insert.append(doc)
            seen_agesuspect_this_run.setdefault(fid, set()).add(sig)
            no_date_report.append({
                "collection": CANTAB_COLLECTION,
                "reason": "無 record_date、僅有年齡，已匯入並標記 age_suspect，"
                          "待後續日期回填管線（famid_timepoint_map.py，尚未實作）處理",
                **doc,
            })
        else:
            sig = build_signature(doc, compare_fields)
            known = existing_nodate_sigs.get(fid, set()) | seen_nodate_this_run.get(fid, set())
            if sig in known:
                no_date_report.append({
                    "collection": CANTAB_COLLECTION,
                    "reason": "無 record_date 亦無年齡，且量表欄位與既有 null-date 紀錄完全相同，略過",
                    **doc,
                })
                continue
            doc["flag_no_date"] = True
            to_insert.append(doc)
            seen_nodate_this_run.setdefault(fid, set()).add(sig)
            no_date_report.append({
                "collection": CANTAB_COLLECTION,
                "reason": "無 record_date 亦無年齡，已匯入並標記 flag_no_date，"
                          "待後續日期回填管線（famid_timepoint_map.py，尚未實作）處理",
                **doc,
            })
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="CANTAB 匯入：讀取 xlsx，三層去重複後匯入 CANTAB collection")
...
def main(argv=None):
    ...
    compare_fields = [c for c in field_rules if c not in CANTAB_HEADER_FIELDS]

    print(f"📄 檔案數：{len(files)}｜collection：{CANTAB_COLLECTION}"
          f"{'｜DRY-RUN（不寫入）' if args.dry_run else ''}")
    print(f"📐 欄位限制來源：{CANTAB_FIELDS_DIR}｜比對欄位：{len(compare_fields)} 個")
    if project_code:
    ...
    for filepath in files:
        ...
        for sheet_name, df in all_sheets.items():
            ...
            total_empty += n_empty
            n_age_suspect = sum(1 for r in no_date if r["doc"].get(RAW_AGE_FIELD) is not None)
            print(f"  [{sheet_name}] 有日期 {len(dated)}｜無日期(有年齡) {n_age_suspect}｜"
                  f"無日期(無年齡) {len(no_date) - n_age_suspect}｜略過(空值) {n_empty}")
    ...
    print("\n===== 匯入結果 =====")
    if args.dry_run:
        print(f"[{CANTAB_COLLECTION}] dry-run：待新增 {len(to_insert)}｜"
        ...
        inserted = 0
        if to_insert:
            db[CANTAB_COLLECTION].insert_many(to_insert)
            inserted = len(to_insert)
        print(f"[{CANTAB_COLLECTION}] 新增 {inserted}｜"
              f"重複略過 {result['exact_dup_count']}｜"
              f"批次內衝突 {sum(len(v) for _, v in result['batch_conflicts'])}｜"
              f"與DB衝突 {len(result['value_conflicts'])}｜"
              f"略過(空值) {total_empty}｜錯誤 {len(errors)}")
