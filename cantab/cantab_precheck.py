...
# -*- coding: utf-8 -*-
"""
CANTAB 匯入前彙整檢查（precheck）。

彙整多個 CANTAB xlsx 資料檔，確認「應該是同一筆紀錄」的資料不會被重複匯入資料庫：
...
用法（從專案根目錄執行）：
  python cantab/cantab_precheck.py <檔案或資料夾>
  python cantab/cantab_precheck.py data_dir/
  python cantab/cantab_precheck.py a.xlsx b.xlsx
  python cantab/cantab_precheck.py data_dir/ -o report.xlsx
...
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "cantab_precheck.xlsx")
...
def load_compare_fields():
    ...
    if not path:
        raise SystemExit(f"Error: 在 {FIELDS_DIR} 找不到 CANTAB 的 fields.json")
...
def read_records(files, json_fields, compare_fields):
    ...
    for filepath in files:
        ...
        for sheet_name, df in all_sheets.items():
            ...
            # age 欄位可能叫 age_CANTAB / age_cantab，統一改名為 age
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="CANTAB 匯入前彙整檢查：依 famid+age 找出重複資料並彙整")
